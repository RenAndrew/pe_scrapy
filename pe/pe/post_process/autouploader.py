...
class AutoUploader(object):
	...
	def after_crawler_done(self, crawler_name):
		csv_download_path = Config().get(crawler_name).get('csv_download_path')
		csv_aggregate_path = Config().get(crawler_name).get('csv_aggregate_path')
		xls_output_path = Config().get(crawler_name).get('xls_output')
